...
$sqlDelete = "DELETE FROM bidang_perusahaan WHERE id = ?";
$stmtDelete = $pdo->prepare($sqlDelete);
$success = $stmtDelete->execute([$id]);

if ($success) {
    echo "<script>alert('Bidang berhasil dihapus!'); window.location.href='?page=" . ($role === 'superadmin' ? 'profil_admin' : 'profil_perusahaan') . "';</script>";
} else {
