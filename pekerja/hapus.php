...
if (!isset($_GET['id'])) {
    ...
    $redirectPage = ($role === 'superadmin') ? 'pekerja_tampil' : 'profil_perusahaan';
...
if (!$data) {
    echo "<script>alert('Data tidak ditemukan!'); window.location.href='?page=pekerja_tampil';</script>";
    exit();
}

// Eksekusi penghapusan
$sqlDelete = "DELETE FROM pekerja WHERE id = ?";
$stmtDelete = $pdo->prepare($sqlDelete);
$success = $stmtDelete->execute([$id]);

if ($success) {
    echo "<script>alert('Data berhasil dihapus!'); window.location.href='?page=" . ($role === 'superadmin' ? 'pekerja_tampil' : 'profil_perusahaan') . "';</script>";
} else {
